// src/components/tienda/productos/ProductRatingsAndComments.js
'use client';
...
export default function ProductRatingsAndComments({ product, onRatingUpdate }) {
  const { user } = useAuth();
  ...
  const checkUserRating = async () => {
    try {
      const ratingsRef = collection(db, 'valoraciones');
      const q = query(
        ratingsRef,
        where('productoId', '==', product.id),
  ...
  const submitRating = async (rating) => {
    ...
    try {
      ...
      if (!existingRatingSnapshot.empty) {
        ...
        await updateDoc(doc(db, 'valoraciones', existingRatingDoc.id), {
          puntuacion: rating,
      ...
      await recalculateProductRatings(product.id);
  ...
  // Función para recalcular las valoraciones del producto
  const recalculateProductRatings = async (productId) => {
    try {
      // Obtener todas las valoraciones del producto
      const ratingsRef = collection(db, 'valoraciones');
      const ratingsQuery = query(ratingsRef, where('productoId', '==', productId));
  ...
  const submitComment = async () => {
    ...
    try {
      ...
      // Actualizar contador de comentarios en el producto
  ...
  const averageRating = product.valoraciones?.promedio || 0;
  const totalRatings = product.valoraciones?.total || 0;
      ...
      {user && (
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 mb-6">
          <h4 className="font-medium text-gray-900 dark:text-white mb-3">
            {userRating > 0 ? 'Actualiza tu valoración' : 'Deja tu valoración'}
          </h4>
          ...
          <textarea
            ...
            rows={3}
            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 resize-none"
          />
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6">
          <p className="text-blue-800 dark:text-blue-300 text-center">
            <a href="/login" className="font-medium hover:underline">
              Inicia sesión
            </a> para valorar y comentar este producto
          </p>
        </div>
      )}
