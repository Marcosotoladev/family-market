...
export default function FeaturedServiceButton({ 
  ...
  const handlePayment = async () => {
    if (!user) {
      alert('Debes iniciar sesión para destacar servicios');
      return;
    }
    ...
    try {
      ...
      const response = await fetch('/api/mercadopago/create-preference', {
        ...
        body: JSON.stringify({
          ...
          serviceName: service.titulo,
          amount: amount,
          type: 'service' // Distinguir de productos
        }),
      });
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6 max-w-md mx-auto">
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
        Haz que tu servicio aparezca en la sección "Servicios Destacados" del home por 7 días
      </p>
      ...
      <div className="mb-6">
        ...
        <div className="flex">
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-r-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
            placeholder="1000"
          />
        </div>
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6">
        <h4 className="font-medium text-blue-900 dark:text-blue-100 mb-2 text-sm">
          Beneficios del servicio destacado:
        </h4>
